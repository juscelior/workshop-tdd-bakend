...
RUN dotnet restore
RUN dotnet build "./src/Wiz.Biblioteca.API/Wiz.Biblioteca.API.csproj" -c Debug
CMD dotnet watch --project src/Wiz.Biblioteca.API/Wiz.Biblioteca.API.csproj run
...
COPY . .
RUN dotnet restore
RUN dotnet build "./src/Wiz.Biblioteca.API/Wiz.Biblioteca.API.csproj" -c Release

FROM build AS publish
RUN dotnet publish "./src/Wiz.Biblioteca.API/Wiz.Biblioteca.API.csproj" -c Release -o /publish

FROM mcr.microsoft.com/dotnet/core/aspnet:3.1-buster-slim AS production
ENV ASPNETCORE_ENVIRONMENT=Staging
WORKDIR /app
COPY --from=publish /publish .
ENTRYPOINT ["dotnet", "Wiz.Biblioteca.API.dll"]
EXPOSE 80