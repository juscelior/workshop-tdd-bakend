...
[Open Library](https://openlibrary.org/) é um projeto para listar para guardar em uma endereço da web um registro para cada livro já publicado. É um objetivo elevado, mas alcançável.
...
A [Central do desenvolvedor](https://openlibrary.org/developers) possui uma parte para listar as APIs disponiveis.
...
Para inicio de conversa, preciso informar que não vamos consumir todas essas APIs. Vamos apenas fazer uso de 2 APIs [Books](https://openlibrary.org/dev/docs/api/books) e [Search](https://openlibrary.org/dev/docs/api/search), respectivamente para realizar uma pesquisa por ISBN e outra para pesquisar pelo titulo.
